...
start = () => {
    ...
    document.getElementById("flowInterval").innerHTML = `* Range from ${min} to ${max} *`;
    return 1;
    } else { document.getElementById("flowInterval").innerHTML = ""; }
    ...
    document.getElementById("flowRest").innerHTML = `* Range from ${min} to ${max} *`;
    return 1;
    } else { document.getElementById("flowRest").innerHTML = ""; }
    ...
    if (countdown == interval) {
        divCounter.textContent = `Seq.: ${(++counter + 1000).toString().substring(1)}`;
...
swaping = (bool) => { return bool ? false : true; };
...
print = (div, swap, countdown) => {
    ...
    if (countdown === 1) { swap ? new Audio("src/2pop.mp3").play() : new Audio("src/1pop.mp3").play(); }
    div.textContent = `${(min + 100).toString().substring(1)}:${(sec + 100).toString().substring(1)}`;
